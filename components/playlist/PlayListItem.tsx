...
const PlayListItem = ({
    ...
    return <li className={styles.item}>
        <Link className={styles.link} href={`/profile/playlists/${playlistData.id}`}>
            <span className={styles.count}>{count}</span>
            {
                ...
                />
                : <Avatar className={styles.img} style={{...imageStyle}}>{playlistData.title.slice(0, 2)}</Avatar>
            }
            <div>
                <h3 className={styles.title}>{playlistData.title}</h3>
                {playlistData.description && <p className={styles.text}>{playlistData.description}</p>}
            </div>
        ...
        <button className={`btn ${styles.button} ${styles.play}`} onClick={playlistClickHandler}>
            <Icon icon='play' />
        </button>
        <button className={`btn ${styles.button} ${styles.delete} ${loading ? "overlay-loading" : ""}`} onClick={playlistDeleteClickHandler}>
            <Icon icon='trash' />
        </button>
    </li>
}
